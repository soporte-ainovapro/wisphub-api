...
GATEWAY = "app.infrastructure.gateways.wisphub_network_gateway.WispHubNetworkGateway"
...
async def test_get_ping_result_endpoint_intermittent(mock_ping, auth_client):
    ...
    data = response.json()
    assert "detail" in data
    assert "intermittent" in data["detail"].lower()
...
async def test_get_ping_result_endpoint_no_internet(mock_ping, auth_client):
    ...
    data = response.json()
    assert "detail" in data
    assert "no internet" in data["detail"].lower()
