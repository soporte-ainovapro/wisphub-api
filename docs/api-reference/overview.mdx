...
Todos los endpoints de esta API (excepto `/health` y `/api/v1/auth/token`) requieren autenticación mediante **JWT Bearer token**.

### Quick Start

1. **Obtén un token** llamando a `POST /api/v1/auth/token` con tus credenciales:
...
2. **Usa el token** en el header `Authorization` de cada solicitud:
...
<Info>
  El Swagger UI en `/docs` tiene soporte integrado para autenticación Bearer. Haz clic en el botón **Authorize** e ingresa tu token.
</Info>
...
- **POST** `/api/v1/auth/token` — Obtener JWT Bearer token (no requiere auth previa)
